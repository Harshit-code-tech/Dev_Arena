...
export const authService = {
  ...
  async login(input: LoginInput): Promise<AuthServiceResult> {
    try {
      ...
      // Firebase validates the password, but DevArena requires a fresh email OTP
      // for every explicit email/password login. No previous OTP verification
      // can be reused to skip this challenge.
  ...
  async syncFirebase(input: SyncFirebaseInput): Promise<AuthServiceResult> {
    try {
      ...
      // Email/password users must complete the fresh DevArena OTP generated by
      // the login endpoint for every login. Existing DevArena JWTs are restored
      // separately by /me and are unaffected by this rule.
      if (identity.provider === "password") {
        return {
          statusCode: 401,
          body: {
            code: "AUTH_OTP_REQUIRED",
            message: "A fresh email verification code is required for every email/password login.",
          },
        };
      }
