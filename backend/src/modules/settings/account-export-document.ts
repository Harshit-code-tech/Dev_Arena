...
function simpleValue(value: unknown): string {
  ...
  if (Array.isArray(value))
    return value.length ? value.map(simpleValue).join(", ") : "None";
  if (typeof value === "object") {
    ...
    return entries.length
      ? entries
          .map(
            ([key, item]) => `${escapeHtml(label(key))}: ${simpleValue(item)}`,
          )
          .join("; ")
      : "None";
  }
...
function definitionList(data: RecordLike) {
  return `<dl class="facts">${Object.entries(data)
    .map(
      ([key, value]) =>
        `<div><dt>${escapeHtml(label(key))}</dt><dd>${simpleValue(value)}</dd></div>`,
    )
    .join("")}</dl>`;
}
  ...
  return `<div class="cards">${projects
    .map((project) => {
      ...
      return `<article class="card"><div class="card-head"><div><span>${simpleValue(project.domain)} project</span><h3>${simpleValue(project.title)}</h3></div><b>${simpleValue(project.status)}</b></div><p>${simpleValue(project.description || "No description provided.")}</p>${github ? `<p class="muted">GitHub: ${github}</p>` : ""}<div class="mini-grid"><span>Project score <strong>${simpleValue(project.projectScore)}</strong></span><span>Sessions <strong>${logs.length}</strong></span><span>Milestones <strong>${milestones.length}</strong></span><span>Shared <strong>${simpleValue(project.isShared)}</strong></span></div>${
        ...
        milestones.length
          ? `<details><summary>Milestones</summary>${table(milestones, [
              ["title", "Milestone"],
              ["status", "Status"],
...
export function renderAccountExportDocument(payload: RecordLike) {
  ...
  const projects = Array.isArray(payload.projects)
    ? (payload.projects as RecordLike[])
    : [];
  const dsa = Array.isArray(payload.dsaHistory)
    ? (payload.dsaHistory as RecordLike[])
    : [];
  const practice = Array.isArray(payload.practiceHistory)
    ? (payload.practiceHistory as RecordLike[])
    : [];
  const fullstack = Array.isArray(payload.legacyFullstackHistory)
    ? (payload.legacyFullstackHistory as RecordLike[])
    : [];
  const notifications = Array.isArray(payload.notifications)
    ? (payload.notifications as RecordLike[])
    : [];
  const achievements = Array.isArray(payload.achievements)
    ? (payload.achievements as RecordLike[])
    : [];
  const activities = Array.isArray(payload.activityHistory)
    ? (payload.activityHistory as RecordLike[])
    : [];
  const weeklyScores = Array.isArray(scoring.weeklyScores)
    ? (scoring.weeklyScores as RecordLike[])
    : [];
  const scoreEvents = Array.isArray(scoring.scoreEvents)
    ? (scoring.scoreEvents as RecordLike[])
    : [];
  const friends = Array.isArray(network.friends)
    ? (network.friends as RecordLike[])
    : [];
  const sentRequests = Array.isArray(network.sentRequests)
    ? (network.sentRequests as RecordLike[])
    : [];
  const receivedRequests = Array.isArray(network.receivedRequests)
    ? (network.receivedRequests as RecordLike[])
    : [];
  const invites = Array.isArray(network.emailInvitations)
    ? (network.emailInvitations as RecordLike[])
    : [];
  const repos = Array.isArray(github.authorizedRepositories)
    ? (github.authorizedRepositories as RecordLike[])
    : [];

  const friendRows = friends.map((item) => ({
    person: person(item.person),
    connectedAt: item.connectedAt,
  }));
  const sentRows = sentRequests.map((item) => ({
    person: person(item.person),
    status: item.status,
    createdAt: item.createdAt,
  }));
  const receivedRows = receivedRequests.map((item) => ({
    person: person(item.person),
    status: item.status,
    createdAt: item.createdAt,
  }));
  ...
  <section><h2>GitHub connection</h2><p class="section-note">Only non-secret GitHub connection and repository information is shown. OAuth tokens and installation credentials are excluded.</p>${definitionList({ connected: github.connected, login: github.login, connectedAt: github.connectedAt })}${table(
    repos,
    [
      ["fullName", "Repository"],
  ...
  <section><h2>DSA history</h2><p class="section-note">Problem-solving evidence recorded in DevArena.</p>${table(
    dsa,
    [
      ["activityDate", "Date"],
      ["problemName", "Problem"],
  ...
  <section><h2>Practice history</h2>${table(practice, [
    ["activityDate", "Date"],
    ["title", "Session"],
    ["type", "Type"],
    ["timeSpent", "Minutes"],
  ...
  ${
    fullstack.length
      ? `<section><h2>Legacy full-stack activity</h2><p class="section-note">Historical full-stack tracking records retained from earlier DevArena versions.</p>${table(
          fullstack,
          [
            ["activityDate", "Date"],
            ["title", "Activity"],
  ...
  <section><h2>Scoring history</h2><p class="section-note">Weekly score summaries and the individual score events that contributed to your Arena Score.</p><h3>Weekly scores</h3>${table(
    weeklyScores,
    [
      ["weekStart", "Week"],
      ["dsaPoints", "DSA"],
      ["projectPoints", "Projects"],
      ["practicePoints", "Practice"],
      ["challengePoints", "Tournament"],
      ["totalScore", "Total"],
      ["activeDays", "Active days"],
    ],
  )}<h3>Score events</h3>${table(scoreEvents, [
    ["occurredAt", "Date"],
    ["category", "Category"],
    ["label", "Activity"],
    ["points", "Points"],
  ])}</section>
  <section><h2>Activity calendar</h2>${table(activities, [
    ["date", "Date"],
    ["isActive", "Active"],
  ])}</section>
  <section><h2>Notifications</h2>${table(notifications, [
    ["createdAt", "Date"],
    ["type", "Type"],
    ["message", "Message"],
    ["isRead", "Read"],
  ...
  <section><h2>Player network</h2><h3>Friends</h3>${table(friendRows, [
    ["person", "Player"],
    ["connectedAt", "Connected"],
  ])}<h3>Sent requests</h3>${table(sentRows, [
    ["person", "Player"],
    ["status", "Status"],
    ["createdAt", "Sent"],
  ])}<h3>Received requests</h3>${table(receivedRows, [
    ["person", "Player"],
    ["status", "Status"],
    ["createdAt", "Received"],
  ])}<h3>Email invitations</h3>${table(invites, [
    ["email", "Email"],
    ["status", "Status"],
    ["createdAt", "Created"],
    ["expiresAt", "Expires"],
  ])}</section>
  <section><h2>Achievements</h2>${table(achievements, [
    ["unlockedAt", "Unlocked"],
    ["title", "Achievement"],
    ["category", "Category"],
  ])}</section>
