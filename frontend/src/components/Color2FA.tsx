...
interface Color2FAProps {
  // If true, we are setting up 2FA for a logged-in user
  isSetup?: boolean;
  onSetupComplete?: () => void;

  // If false, we are verifying a login
  tempToken?: string;
  verifyGrid?: string[];
  onVerifySuccess?: (token: string, deviceToken?: string) => void;
}

type ViewMode = 'grid' | 'otp_confirm' | 'otp_verify';
...
export const Color2FA: React.FC<Color2FAProps> = ({ 
    ...
    if (isSetup) {
      try {
        ...
        const res = await fetch('/api/auth/setup-2fa', {
          ...
          body: JSON.stringify({ userId: user.uid, colorSequence: newSequence })
        });
      ...
      try {
        const res = await fetch('/api/auth/verify-2fa', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tempToken, attemptSequence: newSequence })
        });
        const data = await res.json();
        ...
        if (res.ok) {
          ...
          if (newFails >= 3) {
            ...
            toast.error(`Incorrect sequence. (${newFails}/3 fails)`);
            setStatusMessage(`Incorrect sequence! Try again. (${newFails}/3 fails)`);
            setSequence([]);
          }
        }
      } catch (err) {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(data.message || "Failed to send OTP. Check your email.");
      }
    } catch (err) {
      toast.error("Network error");
    } finally {
  ...
  // Default: Grid View
  const displayColors = isSetup ? MASTER_COLOR_PALETTE : verifyGrid;
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={{ marginTop: '20px', textAlign: 'center' }}>
        <p style={{ fontSize: '14px' }}>Progress: {sequence.length} / {requiredLength}</p>
