    ...
    <main className="player-hub-page animated-page">
      ...
      <div className="hub-section-stage page-reveal" style={{ "--reveal-order": 3 } as CSSProperties}>
        ...
        {activeSection === "projects" && <section className="hub-section">
          ...
          {filteredProjects.length === 0 ? <EmptyState title="Nothing shared yet" copy="Everyone is guarding their spaghetti code like nuclear launch codes. Be the first to share a build!" /> : <div className="shared-project-grid">{filteredProjects.map((project) => <article className="shared-project-card" key={project.id}>
        ...
        {activeSection === "collaboration" && <section className="hub-section">
          ...
          {filteredCollaborations.length === 0 ? <EmptyState title="No co-op missions yet" copy="Looking for a co-founder, code buddy, or someone to blame when production crashes? Post a collab opportunity!" /> : <div className="collaboration-list">{filteredCollaborations.map((post, index) => <article className="collaboration-card" key={post.id}>
